...
> 配套设计文档：`docs/技术设计方案/跨平台简历编辑器-技术设计与架构方案.md`（已沉淀于项目）。
...
- **AC1 门面存在且接口完整**：`js/store/` 下提供 `ResumeStore`，暴露统一方法
  ...
  方法签名与 `docs/技术设计方案/跨平台简历编辑器-技术设计与架构方案.md` §3.3 一致。
- **AC2 三套后端实现到位**：
...
- **AC7 构建文档**：新增 `docs/构建与发版/桌面版构建说明.md`，含：前置依赖（Rust / Xcode CLT / WebView2 / CI 说明）、本地构建命令、CI 出包步骤、凭证放置位置与 `.gitignore` 要求、常见报错排查、以及「macOS 上无法直接产出 Windows msi，须用 Windows 或 CI」的明确说明。
...
- **AC7 文档**：新增 `docs/构建与发版/Android版构建说明.md`，含前置依赖（JDK 17 / Android SDK / NDK / rustup 目标）、本地构建步骤、CI 出包与 artifact 下载、**keystore 生成与保管**（含 `keytool` 命令、绝不入库）、真机安装（`adb install`）、常见报错排查，以及「本机无 SDK 时只能走 CI」的明确说明。
...
| **壳内「导出不落盘」根因定位** | 2026-09-19 | ✅ 已定论（方案就绪，未实施） | 见下方专节与 [docs/技术设计方案/App导出不落盘问题-根因与改造方案.md](../技术设计方案/App导出不落盘问题-根因与改造方案.md)。四种方案（`blob:`/`data:` × 有/无下载处理器）**全部实测失败**；`window.print()` 在 WKWebView 里是**静默空操作** → 桌面端 PDF 的三条路（静默导出 / 打印兜底 / 预览下载）**全是死的** |
...
| **自动发版流水线（release.yml）** | 2026-09-19 | ✅ 通过（**Release v1.1.0 已上线**，AC1–AC10 全闭环） | 推 `v*` 标签或手动一键触发 → 复用 `build-desktop.yml` / `build-android.yml`（`workflow_call`）出三平台 + 安卓包 → 聚合、归一化文件名、生成 `SHA256SUMS.txt` → `gh release create` 创建 **GitHub Release**（含版本号、CHANGELOG 更新说明、安装包表格、系统拦截提示）。<br>**真跑结果**：run 35396634497 **6 个 job 全 success**，Release id `391799043`、**9 个资产全部 uploaded / 119.53 MB**；端到端 `shasum -c` **8/8 OK**。首轮 run 35395365350 暴露 2 个真实缺陷（`gh --template` 无 `divf`；资产名 CJK 被剥）已修。详见 [docs/构建与发版/发版说明.md](../构建与发版/发版说明.md) 与下方「发布产物端到端验证」 |
| T6 P5 发布加固 | — | ⏳ 未开始 | 凭证仍**明文**存 `app_config_dir()/sync.config.json`（钥匙串未接）；**壳内文件落盘命令缺失** —— 且已**实证**在 WKWebView 下静默失败（见 [App导出不落盘问题-根因与改造方案.md](../技术设计方案/App导出不落盘问题-根因与改造方案.md)，该项已单列进 ROADMAP 高优先级）；自动更新/签名/公证/引导页未做 |
...
| 3 | **壳内导出落盘：已确认是缺陷**（不再是「可能」） | 已实证：四种方案（`blob:`/`data:` × 有/无下载处理器）**全部不落盘**；点 PDF 导出还会把编辑器界面**顶掉**；`window.print()` 在 WKWebView 里是**静默空操作** → 桌面端 **PDF 三条路全死**。源码证据：WebKit bug 216918 + `wry-0.55.1/src/wkwebview/mod.rs:572` + Tauri 2.11.5 无 `on_download` | 安装包内「导出 PDF / 长图 / 单文件 HTML / 数据 JSON」**四个功能全部不可用**。方案与 AC 见 [docs/技术设计方案/App导出不落盘问题-根因与改造方案.md](../技术设计方案/App导出不落盘问题-根因与改造方案.md)，**待实施**（阻塞：`js/app.js` 正被「多份简历库」改动） |
| 4 | **T5 冲突合并未做且无 AC** | 无 `baseVersion`；`docs/验收与记录/跨平台改造-验收标准与验收记录.md` 无 T5 章节 | 「手机和电脑同时改」的**不丢数据**保证尚未实现（当前只有「后写覆盖 + 飞书版本历史可回滚」） |
...
| 文件 | 问题 | 处理 |
|---|---|---|
| `docs/AI能力/AI辅助写简历-技术设计方案.md:107` | 举例用了**真实公司名** | 改为 `某公司` |
| `docs/验收与记录/跨平台改造-验收标准与验收记录.md` | 本文件验收叙述里写进了**真实姓名** | 改为「真实姓名特征串」 |
...
**AC2 / AC3 / AC4 ✅** 本机装 Temurin **17.0.20.1**（装法见 `docs/构建与发版/Android版构建说明.md` 第 5 节文末）后：
...
`docs/构建与发版/Android版构建说明.md` 的 `adb uninstall com.resumestudio.app`（×2）与
`docs/构建与发版/桌面版构建说明.md` 的三平台凭证路径 `.../com.resumestudio.app/...`（×3）
...
并在 `docs/构建与发版/Android版构建说明.md` 第 4 节补前置条件警告、第 8 节补对应排错条目
...
补上的**结构级自检**（本机可跑，见 `docs/构建与发版/发版说明.md`）：从 workflow 里抽出补丁脚本，
...
必须改用 `printf '%s\n'` 或直接把密码 base64 后再解码写入。** 已记入 `docs/构建与发版/Android版构建说明.md`。
...
已把三条通道的优先级写进 `docs/构建与发版/Android版构建说明.md` 第 8.1 节。
...
`import java.util.Properties`」这个决定性事实。这一轮改成两步，见 `docs/构建与发版/Android版构建说明.md` 第 5.8 节：
...
（命令见 `docs/构建与发版/Android版构建说明.md` 第 5.8 节）：
...
设计文档与操作手册：[docs/构建与发版/发版说明.md](../构建与发版/发版说明.md)。
...
已全部改成 `${VAR}` 形式，并把这个审计脚本写进了 `docs/构建与发版/发版说明.md` 第 10 节，
作为推送前的固定检查项。
...
> 见 `docs/构建与发版/发版说明.md` 第 5 节：它是**防御性设置**，不是缺陷修复 —— 对照实验已推翻
> 「不加重跑必炸」的推断。
...
**④ Android APK 的签名证书：从公开产物独立复现 CI 的断言（不装 JDK）**
...
**⑤ 各平台产物内部的版本号都等于发布版本（不只是文件名带版本号）**

这一条直接对应需求里的「**要有版本**」—— 文件名带版本号是不够的：用户把包重命名、或系统
在安装后展示的版本、或应用商店读到的版本，都取自**包内部**。逐个拆出来核对：

| 产物 | 内部版本字段（读取方式） | 实测值 | |
|---|---|---|---|
| macOS `dmg` | `Contents/Info.plist` → `CFBundleShortVersionString` | **1.1.0** | ✅ |
| Android `apk` | 二进制 `AndroidManifest.xml` → `android:versionName` | **1.1.0**（`versionCode=1001000`） | ✅ |
| Android `aab` | `base/manifest/AndroidManifest.xml`（protobuf）→ `versionName` | **1.1.0** | ✅ |
| Debian `deb` | `control` 归档 → `Version:` | **1.1.0** | ✅ |
| Windows `setup.exe` | PE `VS_VERSIONINFO` → `ProductVersion` / `FileVersion` | **1.1.0** | ✅ |
| Windows `msi` | MSI 字符串池 → `ProductVersion` 属性 | **1.1.0.0**（MSI 要求四段式，即 1.1.0 的规范形式） | ✅ |
| Linux `AppImage` | 版本在 squashfs 内 | **未直接读到**（本机无 `unsquashfs`；结构已验：ELF64 LSB + `hsqs` 魔数） | ⚠️ |
| `standalone.html` | —— | **包内完全没有版本号**（见下） | ⚠️ |

**顺带发现一个真缺口：`standalone.html` 内部没有任何版本标识。**
全文搜 `1.1.0` / `Resume Studio` / `APP_VERSION` 均 **0 命中** —— 它的版本信息**只存在于文件名**。
后果：用户下载后重命名、或把文件转给别人，就再也无法判断这是哪一版；
而这个文件恰恰是**最可能被转发**的那个（双击即用、无需安装）。
**建议**（本轮未做，`tools/build-single.js` 正在被并行会话改动）：构建时从 `package.json`
注入一个 `<meta name="app-version" content="1.1.0">`，让单文件版自带版本标记。

**另外两项靠这次核对顺手确认的事实：**

- **`minSdkVersion = 24`**（Android 7.0）—— 由 APK 的 `<uses-sdk>` 实测读出。
  这解释了为什么「只有 v2 签名、没有 v1 签名」是**正确**的：v1（JAR 签名）只在
  API < 24 的设备上才被需要。`targetSdkVersion = 36`、`compileSdkVersion = 36`。
- **跨平台标识符一致**：APK/AAB 的 `package` 与 dmg 的 `CFBundleIdentifier`
  **都是 `com.resumestudio.desktop`** —— 早前修过的 identifier 漂移没有再出现。

**⑥ 单文件版在真实浏览器里跑过一遍（本机无服务器、直接 `file://` 双击场景）**

此前只验了它的内容、自足性与 PII，**没真开过**。这次用真实 Chromium 打开：

| 判据 | 结果 |
|---|---|
| 页面标题 | `林一帆 · 简历编辑器` —— 标题由数据推导，**证明应用启动且读到了数据** |
| 关键全局 | `ResumeEditor` / `ResumeAudit` / `ResumeExport` / `ResumeStore` **全部 `object`**（若初始化抛错，这些会在 IIFE 末尾赋值失败而变 `undefined`） |
| 预览渲染 | `.resume` 容器存在且含 `林一帆`、正文 4780 字符、**3 张图片**（与体检面板报的「3 张图片」一致） |
| **页面错误** | **0 条** |
| **控制台输出** | **0 条**（干净启动） |
| **外部资源引用** | `<script src>` **0 个**、`<link href>` **0 个**；网络层实测**无任何 http(s) 请求** |
| 核心 Web Vitals | FCP/LCP **52ms**、CLS **0** |

网络层还看到两处 `file://` 请求失败（`data/resume.json`、`template.json`，因为 HTML 旁边没有它们）
和一次 `POST /api/resume` —— 这正是**文档里声明的降级路径**（`file://` 下读取失败则回退、
写回静默失败）。**回退到内联 `DEMO_DATA` 确实生效**：标题与预览内容都是 `林一帆`，
而不是空骨架。⇒ 「单文件版双击即用、无需服务器」这句话是成立的。

**⑦ 文档里一处失实数字已更正**

`docs/构建与发版/发版说明.md` 曾写「Android APK 约 **140 MB+**」。实测 **24.7 MB**，
...
- Windows `.exe` / `.msi`、Linux `.deb` / `.AppImage`、Android `.apk` **未做实物运行验证**
  （本机是 macOS，无对应环境）。**但内部版本号已全部读出来核对过（见 ⑤）** ——
  这比「只看文件名」强得多，仍不等于「装上去能跑」；
- **`AppImage` 的内部版本号未读到**（本机无 `unsquashfs`），只验了结构（ELF64 + `squashfs` 魔数）；
- **`standalone.html` 包内没有版本号**（见 ⑤），建议后续构建时注入；
