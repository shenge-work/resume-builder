...
| Android 包是 debug 签名的 | 见 `docs/构建与发版/Android版构建说明.md` 第 5 节。`release.yml` 已用 `secrets: inherit` 传递签名凭据，若仍异常，注意被调用 workflow 是否漏了 `secrets: inherit` |
...
| `npm run desktop:build` 卡在 `bundle_dmg.sh` 报 `grep: bad regex ... brackets not balanced` 或 `osascript ... 权限违例 (-10004)` | 两类**环境限制**（非代码缺陷）：① 沙箱把 `grep` 换成 toybox，其 `grep --color=never` 仍输出 ANSI 颜色码 → create-dmg 的 `find_mount_dir` 正则崩坏；② headless/CI 无 Finder 自动化权限 → create-dmg 的 osascript 美化步骤报 -10004。`.app` 与 release 二进制此时**已正常产出**，只差 dmg 最后一步。解法：跑 `npm run desktop:dmg`（`tools/build-dmg.js`）—— 复用已产出的 `.app`，把脚本内 `grep` 替换为 `/usr/bin/grep` + 追加 `--skip-jenkins`，产出功能完整、仅无自定义背景/图标定位的 dmg |
...
> 方法见 `docs/构建与发版/Android版构建说明.md` 第 5.8 节。
>
...
- [`docs/构建与发版/Android版构建说明.md`](./Android版构建说明.md) —— Android 构建与签名（含 4 个 secrets 的准备与排错）
- [`docs/构建与发版/桌面版构建说明.md`](./桌面版构建说明.md) —— 桌面三端构建
- [`CHANGELOG.md`](../../CHANGELOG.md) —— 版本变更记录（Release 说明的来源）
- [`docs/验收与记录/跨平台改造-验收标准与验收记录.md`](../验收与记录/跨平台改造-验收标准与验收记录.md) —— 逐任务的验收标准与实测记录
